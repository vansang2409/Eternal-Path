...
export class Hud {
  ...
  private renderStatsModal(): void {
    ...
    // Sprint 286: paragon block with a kill-progress bar.
    const pv = paragonProgressView(p.paragonPoints ?? 0, p.paragonProgress ?? 0);
    const pct = pv.maxed ? 100 : Math.round((pv.into / Math.max(1, pv.needed)) * 100);
    const paragonBlock = `
      <div style="padding:8px 10px;margin:10px 0 0;border-radius:6px;background:rgba(34,24,44,0.6);border:1px solid #5a3a7a">
        <div style="display:flex;justify-content:space-between;font-size:12px"><strong style="color:#c79bff">🔮 Cảnh Giới ${pv.points}/${PARAGON_MAX_POINTS}</strong><span style="color:#d6dddf">${pv.maxed ? "TỐI ĐA" : `${pv.into}/${pv.needed} kills`}</span></div>
        <div style="height:6px;border-radius:3px;background:#222;overflow:hidden;margin-top:5px"><div style="height:100%;width:${pct}%;background:linear-gradient(to right,#6e4c9b,#c79bff)"></div></div>
        <div style="font-size:10px;color:#9aa;margin-top:3px">Mỗi 100 quái: vĩnh viễn +1 công, +5 HP. Đã cộng: +${pv.points} công, +${pv.points * 5} HP.</div>
      </div>`;
    body.innerHTML = `<p style="color:#d6dddf;font-size:12px;margin:0 0 10px">Hành trình của <strong style="color:#ffd166">${escapeHtml(p.accountName)}</strong> — cấp ${p.stats.level}.</p>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">${rows.map(([k, v]) => `<div style="display:flex;justify-content:space-between;gap:8px;padding:7px 10px;border-radius:6px;background:rgba(28,28,28,0.5);border:1px solid #2a2a2a;font-size:12px"><span style="color:#9aa">${k}</span><strong style="color:#f1f1f1">${v}</strong></div>`).join("")}</div>${paragonBlock}`;
  }
